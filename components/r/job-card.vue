<template>
  <div
    class="bg-remark-light p-4 rounded-lg shadow-md hover:shadow-xl duration-300 mb-4 w-full"
  ...
  >
    <VFlex justifyContent="space-between">
      <div class="w-full">
        <a
          target="_blank"
          :href="'/jobs/job-' + slug"
          class="text-lg font-bold hover:underline duration-200 line-clamp-1 text-ellipse"
        >
          {{ title }}
        </a>
    ...
    <VFlex>
      <img
        ...
        :src="
          companyLogo != ''
            ? companyLogo
            : 'https://visko-hr-pvt-ltd.s3.ap-south-1.amazonaws.com/remark/remark-placeholder.jpg'
        "
      />
      <VGap :width="10" />
      <div class="w-full">
        <a
          href="#"
          target="_blank"
          class="text-md font-normal hover:underline line-clamp-1 text-ellipse"
        >
          {{ companyName }}
        </a>
        <VFlex justifyContent="start">
          ...
          <VGap :width="4" />
          <p class="text-slate-600 text-sm line-clamp-1 text-ellipse w-full">
            {{ companyLocation }}
          </p>
        </VFlex>
      </div>
    ...
    <div class="flex flex-wrap md:flex-none justify-center md:justify-around">
      ...
      <div
        class="w-3/12 md:w-3/12 px-0 md:px-3 py-1 my-1 md:my-0 bg-slate-0 rounded"
      >
        <VIconText
          gap="2"
          icon="mdi:briefcase"
      ...
      <div
        class="hidden md:block w-10/12 md:w-3/12 px-0 md:px-3 py-1 my-1 md:my-0 bg-slate-0 rounded"
      >
        <VIconText
          gap="2"
      ...
      <VFlex class="gap-x-5" justifyContent="space-between">
        <div class="w-full flex gap-x-3">
          ...
          <a
            href="#"
            v-if="
              userStore?.user?.isLogged && userStore?.user?.user_type == '1'
            "
          >
            <VIconText
              gap="2"
              class="text-md"
              :class="isSaved == '1' ? 'text-teal-800' : 'text-slate-400'"
