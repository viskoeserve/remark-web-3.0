        ...
        <div class="w-full md:w-6/12 py-2 px-2">
          <VContainer class="border p-5 rounded-lg">
            <VFlex justifyContent="space-between">
              <VContainer :width="100">
                ...
                <VIconText
                  ...
                  gap="1"
                  :text="jobStore.job?.company?.company_address"
                />
                <VGap :height="5" />
                <VIconText
                  ...
                  :text="
                    'Rs. ' +
                    jobStore.job?.job_minimum_salary +
                    ' to ' +
                    'Rs. ' +
                    jobStore.job?.job_maximum_salary +
                    ' (Per Month)'
                  "
                />
              </VContainer>
            ...
            <VContainer>
              ...
              <VGap :height="5" />
              <VFlex
                v-if="jobStore.job?.job_ext_experience?.length > 0"
                justifyContent="start"
                class="gap-x-3 text-start flex-wrap"
              >
                <div
                  v-for="(exp, i) in jobStore.job.job_ext_experience"
                  :width="30"
                  class="p-3 my-1 bg-remark-light rounded border border-teal-300"
                  :key="i"
                >
                  <p class="font-semibold">{{ exp.ExperienceTitle }}</p>
                  <span class="text-sm text-slate-500">
                    {{ exp.ExperienceMonth }} Month(s)
                    {{ exp.ExperienceYear }} Year(s)
                  </span>
                </div>
              </VFlex>
            ...
            <VDivider
              v-if="jobStore.job?.job_key_skills?.length > 0"
              :height="15"
            />
            ...
            <VContainer v-if="jobStore.job?.job_key_skills?.length > 0">
              ...
              >
                <div v-for="(skill, i) in jobStore.job.job_key_skills" :key="i">
                  <div
                    v-if="skill != ''"
                    class="px-5 py-2 my-2 bg-remark-light rounded-full border border-teal-300"
                  >
            ...
            <VFlex justifyContent="start" class="gap-x-2">
              ...
              <VContainer>
                <p class="text-xl font-bold">
                  {{ jobStore.job?.company.company_name }}
                </p>
          ...
          <VContainer class="border p-5 rounded-lg">
            ...
            <p class="text-sm my-2">
              ...
              {{ jobStore.job.job_schedule }}
            </p>
            <p class="text-sm my-2">
              <span class="font-semibold">Education:</span>
              {{ jobStore.job.job_education }}
            </p>
            <p class="text-sm my-2">
              <span class="font-semibold">Qualification:</span>
              {{ jobStore.job.job_qualification }}
            </p>
            <p class="text-sm my-2">
              <span class="font-semibold">Hiring:</span>
              {{ jobStore.job.job_hiring_count }}
            </p>
          </VContainer>
          <VGap :height="20" />
          <VContainer class="border p-5 rounded-lg">
