<template>
  ...
  <div
    :class="
      showFilter ? 'w-screen' : 'w-full' + isLoadingJobs ? ' pt-0' : 'pt-5'
    "
    class="h-full px-2 pt-5 md:px-10"
  >
    <VFlex
      v-if="!isLoadingJobs"
      alignItem="start"
      justifyContent="center"
      class="gap-x-3 flex-wrap md:flex-none w-full"
    >
      <div
        v-if="showFilter && jobs.length > 0"
        :style="{ height: '800px' }"
        :class="showInMobile ? 'block' : 'hidden'"
        class="bg-remark-light md:block rounded-lg shadow-xl sticky top-0 px-3 w-full md:w-3/12"
      >
        <VContainer :width="80" class="px-1 py-5">
          <div class="flex justify-between items-center">
            ...
            <div class="block md:hidden">
              <button
                class="py-2 px-4 bg-red-600 text-white rounded-full"
        ...
        <VContainer
          :width="100"
          :style="{ height: '300px' }"
          class="block overflow-y-scroll border p-3 rounded-xl border-teal-100"
        >
          <VContainer class="px-3">
            <VFlex justifyContent="space-between">
              ...
              <a
                ...
                class="text-teal-800 font-bold text-sm"
                >Selected ({{
                  filterSkills.all.filter((sk) => sk.selected == true).length
                }})</a
              >
            ...
            <div class="flex flex-wrap gap-x-2 h-auto">
              <VChip
                @click="quickSelectSkill(skill)"
                v-for="(skill, i) in filterSkills.all"
                :key="i"
                class="my-1"
                :bgColor="
                  skill.selected ? 'bg-teal-800 text-white' : 'bg-teal-100'
                "
              >
            ...
            <VContainer
              ...
              class="absolute top-0 p-3 bg-white rounded-xl shadow-xl"
            >
        ...
        <VContainer
          :style="{ height: '180px' }"
          class="block p-3 border border-teal-100 rounded-xl overflow-y-scroll"
        >
          <VFlex justifyContent="space-between">
            <p class="font-semibold">Schedule</p>
            <a href="javascript:void(0)" class="text-teal-800 font-bold text-sm"
              >Selected ({{
                filterSchedule.all.filter((fl) => fl.selected == true).length
              }})</a
            >
            <!-- <a href="javascript:void(0)" class="text-teal-800 font-bold text-sm underline" @click="toggleShowAllSkills" >See all</a> -->
          </VFlex>
          <VFlex :style="{ height: '100px' }" class="gap-x-2 flex-wrap">
            <VChip
              @click="quickSelectSchedule(schedule)"
              v-for="(schedule, i) in filterSchedule.all"
              :key="i"
              class="my-1"
              :bgColor="
                schedule.selected ? 'bg-teal-800 text-white' : 'bg-teal-100'
              "
            >
              <p class="text-sm">
                {{ schedule.schedule }} ({{ schedule.count }})
              </p>
            </VChip>
          </VFlex>
        </VContainer>
        ...
        >
          <VFlex justifyContent="space-between">
            ...
            <a href="javascript:void(0)" class="text-teal-800 font-bold text-sm"
              >Selected ({{
                filterLocations.all.filter((fl) => fl.selected == true).length
              }})</a
            >
            <!-- <a href="javascript:void(0)" class="text-teal-800 font-bold text-sm underline" @click="toggleShowAllSkills" >See all</a> -->
          </VFlex>
          <VFlex :style="{ height: '100px' }" class="gap-x-2 flex-wrap">
            <VChip
              @click="quickSelectLocation(location)"
              v-for="(location, i) in filterLocations.all"
              :key="i"
              class="my-1"
              :bgColor="
                location.selected ? 'bg-teal-800 text-white' : 'bg-teal-100'
              "
            >
              <p class="text-sm">
      ...
      <div class="w-full" :class="!showFilter ? 'md:w-6/12' : 'md:w-5/12'">
        <VContainer class="">
          <VFlex
            :width="100"
            class="bg-remark-light px-8 py-2 shadow-xl hover:shadow-none duration-200"
          ...
          >
            ...
            <button
              paddingY="3"
              @click="filterJobs(1)"
              class="bg-teal-800 p-3 rounded-full text-white"
            >
        ...
        <VContainer
          v-if="isSearched && jobs.length > 0"
          class="mx-3 font-semibold w-full"
        >
          <Head>
            <Title
              >{{ countJobs }} {{ searchTitle }} jobs in
              {{ filter.locations[0] }} location</Title
            >
          </Head>
          <VFlex class="gap-x-2 mx-3" justifyContent="space-between">
            <p>
              {{ countJobs }} {{ searchTitle }} jobs in
              {{ filter.locations[0] }} location
            </p>
            <div
              @click="showInMobile = !showInMobile"
              class="block md:hidden text-3xl text-teal-800"
            >
      ...
      <div class="w-full md:w-3/12 sticky md:h-screen top-0">
        <VContainer :width="100" class="bg-remark-light h-screen shadow-xl">
          ...
          <VContainer class="text-center md:px-5 md:text-sm py-2">
            <p class="text-lg md:text-md px-4 font-bold">
              Check Latest Jobs on Application
            </p>
            <p class="text-slate-400 md:text-sm text-sm">
...
export default {
  ...
  methods: {
    ...
    async filterJobs(pageNum) {
      ...
      if (data?.value.status) {
        ...
        data.value?.jobs.forEach((job) => {
          ...
          job.job_ext_experience =
            job.job_ext_experience != ""
              ? JSON.parse(job.job_ext_experience)
              : [];
          if (job.job_ext_experience.length > 0) {
            job.is_experienced = true;
          } else {
        ...
        this.setToStorage(
          ...
          this.countJobs,
          true,
          JSON.stringify(data.value.filter)
        );
      }
      ...
      this.filterSchedule.all.forEach((sch, inde) => {
        if (schedule.schedule == "All") {
          ...
          if (sch.schedule == "All") {
            sch.selected = false;
            sch.tempSelected = false;
            this.filter.schedules = this.filter.schedules.filter(
              (s) => s != "All"
            );
          }
      ...
      this.filterLocations.all.forEach((loc, inde) => {
        if (location.location == "All") {
          ...
          if (loc.location == "All") {
            loc.selected = false;
            loc.tempSelected = false;
            this.filter.locations = this.filter.locations.filter(
              (s) => s != "All"
            );
          }
    ...
    doReadyScheduleFilter(schedules, filters) {
      ...
      if (schedules.length > 0) {
        ...
        this.storage.setItem(
          "filterSchedule",
          JSON.stringify(this.filterSchedule)
        );
      }
    },
    ...
    doReadyLocationFilter(locations, filters) {
      ...
      this.storage.setItem(
        "filterLocations",
        JSON.stringify(this.filterLocations)
      );
    },
  },
...
<script setup>
definePageMeta({
  title: "Remark Jobs | Best Job Search Site in India | Free Job Alert",
  meta: [
    {
      name: "description",
      content: "Best job search site in India. Explore top job opportunities & vacancies for freshers with Remark Jobs. Free job alerts & recommendations. AI-powered searches.",
    },
    {
      name: "keywords",
      content: "job portal, india job portal, job apply application, job portal sites in india, application regarding job, Free Job Posting Portals in India",
    },
